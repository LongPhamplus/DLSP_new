<template>
  <div class="min-h-screen bg-gray-50">
        ...
        <div class="flex justify-between items-center">
          <div class="flex items-center space-x-4">
            <button 
              ...
              class="flex items-center space-x-2 px-3 py-2 text-gray-600 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors"
            >
          ...
          <div class="flex items-center space-x-4">
            <span class="text-sm text-gray-600">{{ $t('admin.dashboard.welcome') }}, {{ postsStore.currentUser?.username }}</span>
    ...
    <main class="container-custom py-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow p-6">
          <div class="text-gray-500 text-sm font-medium">Tổng số bài viết</div>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex justify-end space-x-2">
                  <button 
                    ...
                    :title="post.published ? 'Hủy xuất bản' : 'Xuất bản'"
    ...
    <!-- Add/Edit Modal -->
    <div v-if="showAddModal || editingPost" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto p-6">
        ...
        <form @submit.prevent="savePost" class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('admin.dashboard.form.titleEn') }}</label>
              <input v-model="formData.title" type="text" required class="input-field" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('admin.dashboard.form.titleVi') }}</label>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('admin.dashboard.form.excerptEn') }}</label>
              <textarea v-model="formData.excerpt" rows="3" required class="input-field"></textarea>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('admin.dashboard.form.excerptVi') }}</label>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('admin.dashboard.form.contentEn') }}</label>
              <textarea v-model="formData.content" rows="8" required class="input-field"></textarea>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('admin.dashboard.form.contentVi') }}</label>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('admin.dashboard.form.category') }}</label>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('admin.dashboard.form.author') }}</label>
