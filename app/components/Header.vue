<template>
  ...
  <header class="relative z-60 bg-white/95 backdrop-blur-md shadow-lg" style="position: relative; z-index: 60;">
    <div class="container-custom flex items-center justify-between py-4">
      ...
      <NuxtLink to="/" class="flex items-center gap-3 hover:opacity-80 transition-opacity">
        ...
        />
        <span class="text-xl font-bold text-green-600 hidden sm:inline">{{ $t('hero.title') }}</span>
      </NuxtLink>
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          @click="toggleMenu"
          class="md:hidden bg-green-600 hover:bg-green-700 text-white p-2.5 rounded-lg transition-all duration-300"
  ...
  <div v-if="!isAtTop" class="fixed top-0 left-0 right-0 z-50">
    ...
    <div class="fixed top-2 sm:top-4 left-2 sm:left-4 z-50">
      <NuxtLink to="/" class="flex items-center justify-center bg-white/10 backdrop-blur-sm p-2 sm:p-4 rounded-full hover:shadow-xl transition-all duration-300 hover:scale-110">
        <img 
          src="/images/Sapa_logo.png" 
          alt="Sapa Paragliding Logo" 
          class="h-12 sm:h-20 w-12 sm:w-20 object-contain"
          @error="handleLogoError"
        />
      </NuxtLink>
    </div>
    ...
    <div class="fixed top-2 sm:top-4 right-2 sm:right-4 z-50 flex flex-col items-center gap-2 sm:gap-3">
      ...
      <button
        @click="toggleMenu"
        class="bg-white hover:bg-gray-100 text-green-600 p-2.5 sm:p-4 rounded-full shadow-lg transition-all duration-300 hover:shadow-xl"
        aria-label="Toggle menu"
      >
        <svg
          v-if="!isMenuOpen"
    ...
    <Transition name="slide">
      <nav
        v-if="isMenuOpen"
        class="fixed top-0 left-0 h-full w-full sm:w-80 bg-white shadow-2xl overflow-y-auto z-50"
      >
        <div class="p-4 sm:p-8">
          ...
          <div class="mb-6 sm:mb-8">
            <h2 class="text-xl sm:text-2xl font-bold text-green-600">{{ $t('hero.title') }}</h2>
          </div>
          ...
          <ul class="space-y-2 sm:space-y-4 mb-6 sm:mb-8">
            <li v-for="item in menuItems" :key="item.path">
              <NuxtLink
                :to="getLocalizedPath(item.path)"
                @click="closeMenu"
                class="block py-2.5 sm:py-3 px-3 sm:px-4 text-gray-700 hover:bg-green-50 hover:text-green-600 rounded-lg transition-all duration-200 font-medium text-sm sm:text-base"
          ...
          <div class="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t">
            ...
            <div class="flex space-x-4">
              <a
                href="https://facebook.com/sapaparagliding"
                target="_blank"
              ...
              <a
                href="https://instagram.com/sapaparagliding"
                target="_blank"
                rel="noopener noreferrer"
                class="text-gray-600 hover:text-green-600 transition-colors"
              ...
              <a
                href="https://tiktok.com/@sapaparagliding"
                target="_blank"
                rel="noopener noreferrer"
                class="text-gray-600 hover:text-green-600 transition-colors"
                aria-label="TikTok"
...
// Get localized path for menu items
const getLocalizedPath = (path: string) => {
  return locale.value === 'vi' ? path : `/${locale.value}${path}`
}
