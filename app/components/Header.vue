  ...
  <header class="relative z-60 bg-white/95 backdrop-blur-md shadow-lg" style="position: relative; z-index: 60;">
    <div class="container-custom flex items-center justify-between py-4">
      ...
      <NuxtLink to="/" class="flex items-center gap-3 hover:opacity-80 transition-opacity">
        ...
        <span class="text-3xl font-black text-green-600 hidden sm:inline uppercase tracking-wide">{{ $t('hero.title')
        }}</span>
      </NuxtLink>
      ...
      <nav class="hidden md:flex items-center gap-8">
        <NuxtLink v-for="item in menuItems" :key="item.path" :to="getLocalizedPath(item.path)"
          class="uppercase text-gray-700 hover:text-green-600 font-medium transition-colors" active-class="text-green-600">
          {{ $t(item.label) }}
        </NuxtLink>
      </nav>
          ...
          <ul class="space-y-2 sm:space-y-4 mb-6 sm:mb-8">
            <li v-for="item in menuItems" :key="item.path">
              <NuxtLink :to="getLocalizedPath(item.path)" @click="closeMenu"
                class="uppercase block py-2.5 sm:py-3 px-3 sm:px-4 text-gray-700 hover:bg-green-50 hover:text-green-600 rounded-lg transition-all duration-200 font-medium text-sm sm:text-base"
          ...
          <div class="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t">
            ...
            <div class="flex space-x-4">
              <a href="https://facebook.com/sapaparagliding" target="_blank" rel="noopener noreferrer"
                class="text-gray-600 hover:text-green-600 transition-colors" aria-label="Facebook">
              ...
              <a href="https://instagram.com/sapaparagliding" target="_blank" rel="noopener noreferrer"
                class="text-gray-600 hover:text-green-600 transition-colors" aria-label="Instagram">
                <svg class="w-5 h-5 sm:w-6 sm:h-6" fill="currentColor" viewBox="0 0 24 24">
                  <path
              ...
              <a href="https://tiktok.com/@sapaparagliding" target="_blank" rel="noopener noreferrer"
                class="text-gray-600 hover:text-green-600 transition-colors" aria-label="TikTok">
                <svg class="w-5 h-5 sm:w-6 sm:h-6" fill="currentColor" viewBox="0 0 24 24">
                  <path
