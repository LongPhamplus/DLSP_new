  ...
  <div class="space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            ...
            <div>
              <p class="text-xs text-gray-600 font-medium uppercase tracking-wide">{{ $t('booking.step1Details.numberOfPassengers') }}</p>
              <p class="text-xl font-bold text-gray-900">{{ numberOfPassengers }} {{ $t('booking.step1Details.people') }}</p>
            </div>
          </div>
          ...
          <div class="flex items-center bg-white rounded-lg shadow-sm p-0.5 border border-gray-200">
            ...
            <button
              ...
              :disabled="numberOfPassengers >= 20"
              class="w-9 h-9 rounded-md bg-gray-50 hover:bg-green-600 hover:text-white text-gray-700 disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center font-bold text-lg"
            >
              +
            </button>
        ...
        <div class="flex items-center justify-between bg-white rounded-lg p-4 border border-green-200">
          <div>
            <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">{{ $t('booking.step1Details.estimatedTotal') }}</p>
    ...
    <div class="bg-gray-50 rounded-xl p-6 space-y-6">
      ...
      <div>
        ...
        <p class="text-lg text-gray-600">({{ formatPriceUSD(basePriceUSD) }}{{ $t('booking.step1Details.perFlight') }})</p>
      </div>
      ...
      <div>
        ...
        <div class="space-y-3">
          <label
            v-for="option in optionalServices"
            :key="option.id"
          ...
          >
            ...
            <div class="ml-3 flex-1">
              ...
              <div v-if="selectedOptions.includes(option.id) && (option.id === 'drone' || option.id === 'camera360')" class="mt-3 flex items-center gap-3">
                <span class="text-sm text-gray-700 font-medium">{{ $t('booking.step1Details.numberOfPeople') }}:</span>
                <div class="flex items-center bg-gray-100 rounded-full p-1">
                  <button
                    @click.prevent="decreaseServiceQuantity(option.id)"
                  ...
                  <button
                    @click.prevent="increaseServiceQuantity(option.id)"
              ...
              <p class="text-lg font-bold text-green-600 mt-2">
                {{ formatPriceVND(option.priceVND * (selectedOptions.includes(option.id) && (option.id === 'drone' || option.id === 'camera360') ? serviceQuantities[option.id] || 1 : 1)) }}/{{ option.unit }}
                <span class="text-sm text-gray-500">({{ formatPriceUSD(option.priceUSD * (selectedOptions.includes(option.id) && (option.id === 'drone' || option.id === 'camera360') ? serviceQuantities[option.id] || 1 : 1)) }}/{{ option.unit }})</span>
              </p>
            </div>
          </label>
        </div>
        ...
        <div class="space-y-3">
          <div class="flex items-start">
            ...
            <div>
              ...
              <a href="https://maps.app.goo.gl/bGtKFTuxyZvJhsJZ9" target="_blank" class="text-green-600 hover:underline text-sm">
          ...
          <div class="flex items-start">
            ...
            <div>
              ...
              <a href="https://maps.app.goo.gl/mYnh4KJVk3aQZLYC6" target="_blank" class="text-green-600 hover:underline text-sm">
                {{ $t('booking.step1Details.viewMap') }}
              </a>
            </div>
          </div>
      ...
      <div class="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-300 rounded-xl p-6">
        ...
        <ul class="space-y-2">
          ...
          >
            <div class="flex items-center">
              <svg
                v-if="numberOfPassengers >= promo.minPeople"
        ...
        <div v-if="activeDiscount" class="mt-4 p-4 bg-green-600 text-white rounded-lg">
          <p class="font-bold text-lg">{{ $t('booking.step1Details.youSave') }} {{ formatPriceVND(activeDiscount * numberOfPassengers) }}</p>
    ...
    <div class="bg-white border-2 border-gray-200 rounded-xl p-6">
      ...
      <div class="space-y-3">
        <div class="flex justify-between text-gray-700">
          <span>{{ $t('booking.step1Details.flightPriceFor') }} ({{ numberOfPassengers }} {{ $t('booking.step1Details.people') }})</span>
