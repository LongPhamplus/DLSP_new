  ...
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
      ...
      <div class="bg-green-50 p-6 border-b">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p class="font-semibold">{{ bookingData.preferredTime || $t('booking.step4Details.flexible') }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="p-6 border-b">
        <h3 class="text-xl font-bold text-gray-900 mb-4">{{ $t('booking.step4Details.contactInformation') }}</h3>
        <div class="space-y-2">
      ...
      <div class="p-6 border-b">
        <h3 class="text-xl font-bold text-gray-900 mb-4">{{ $t('booking.step4Details.passengerInformation') }}</h3>
        <div class="space-y-3">
          <div
            ...
            :class="expandedPassenger === index ? 'bg-green-50 border-green-300' : 'bg-white hover:bg-gray-50'"
          >
            <!-- Accordion Header -->
            <button
              @click="togglePassenger(index)"
            ...
            >
              ...
              <svg
                ...
                :class="expandedPassenger === index ? 'transform rotate-180' : ''"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <div v-if="expandedPassenger === index" class="px-4 pb-4">
                <div class="grid grid-cols-2 gap-3 text-sm bg-white p-4 rounded-lg">
                  ...
                  <div>
                    ...
                    <p class="font-medium text-gray-900 mt-1">{{ formatDate(passenger.dateOfBirth) }}</p>
                  </div>
                  <div>
      ...
      <div class="p-6 bg-gray-50">
        ...
        <div class="space-y-2">
          ...
          <div v-if="bookingData.discount > 0" class="flex justify-between text-green-600">
            <span>{{ $t('booking.step4Details.discount') }} ({{ Math.round(bookingData.discount * 100) }}%):</span>
            <span>-{{ formatPrice(bookingData.servicePrice * bookingData.numberOfPassengers * bookingData.discount) }}</span>
          </div>
          <div class="border-t pt-2 flex justify-between text-2xl font-bold text-gray-900">
            <span>Total:</span>
            <span class="text-green-600">{{ formatPrice(bookingData.totalPrice) }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl p-6 shadow">
      <div class="flex items-start">
        ...
        <label for="terms" class="ml-3 text-sm text-gray-700">
          ...
          <a href="/Mien tru trach nhiem bay doi 2025 Sapa.pdf" target="_blank" rel="noopener noreferrer" class="text-green-600 hover:underline">{{ $t('booking.terms.termsAndConditions') }}</a>
          {{ $t('booking.terms.and') }}
          <a href="/Mien tru trach nhiem bay doi 2025 Sapa.pdf" target="_blank" rel="noopener noreferrer" class="text-green-600 hover:underline">{{ $t('booking.terms.privacyPolicy') }}</a>
        </label>
      </div>
    </div>
