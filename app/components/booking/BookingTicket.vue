<template>
  <div class="space-y-6">
    <!-- Ticket Preview -->
    <div ref="ticketRef"
      class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl shadow-2xl overflow-hidden border-2 border-green-200">
      <!-- Ticket Header -->
      <div class="bg-gradient-to-r from-green-600 to-emerald-600 text-white p-8">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-green-100 text-sm mt-1">{{ $t('booking.ticket.subtitle') }}</p>
            </div>
          </div>
          <div class="text-right">
            <p class="text-sm text-green-100">{{ $t('booking.ticket.bookingId') }}</p>
        ...
        <div class="border-t border-green-400 pt-4 mt-4">
          ...
          <p class="text-sm text-green-100">{{ companyInfo }}</p>
        </div>
      </div>
      ...
      <div class="p-8 space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
              ...
              <div class="bg-white rounded-lg p-4 space-y-3">
                ...
                <div class="flex justify-between items-center">
                  ...
                  <span class="font-bold text-gray-900">{{ formatDate(bookingData.preferredDate) }}</span>
                </div>
                <div class="flex justify-between items-center">
          ...
          <div class="space-y-6">
            <div>
              ...
              <div class="bg-white rounded-lg p-4 space-y-3">
                <div>
                  <p class="text-xs text-gray-500 uppercase">{{ $t('booking.step2Details.contactName') }}</p>
                  <p class="font-bold text-gray-900">{{ bookingData.contactName }}</p>
                </div>
                <div>
                  <p class="text-xs text-gray-500 uppercase">{{ $t('booking.step2Details.email') }}</p>
                  <p class="font-mono text-sm text-gray-900">{{ bookingData.email }}</p>
                </div>
                <div>
                  <p class="text-xs text-gray-500 uppercase">{{ $t('booking.step2Details.phone') }}</p>
        ...
        <div class="bg-white rounded-lg p-6 border-2 border-green-200">
          <div class="space-y-3">
            ...
            <div v-if="bookingData.discount > 0" class="flex justify-between items-center text-green-600">
              <span>{{ $t('booking.step1Details.discount') }} ({{ Math.round(bookingData.discount * 100) }}%)</span>
              <span class="font-semibold">-{{ formatPrice(bookingData.servicePrice * bookingData.numberOfPassengers *
            ...
            <div class="border-t-2 border-gray-200 pt-3 flex justify-between items-center">
              ...
              <span class="text-2xl font-bold text-green-600">{{ formatPrice(bookingData.totalPrice) }}</span>
            </div>
          </div>
        </div>
